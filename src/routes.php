...
return function (Framework $framework): void {
    ...
    $framework->app()->get('/[{index}]', IndexAction::class)
        ->setName('index');

    $framework->app()->get('/{index}/manifest', ManifestAction::class)
        ->setName('manifest');

    $framework->app()->get('/{index}/canvas[/{canvas}]', CanvasAction::class)
        ->setName('canvas');

    $framework->app()->get('/{index}/diva', DivaViewerAction::class)
        ->setName('diva');

    $framework->app()->get('/{index}/mirador', MiradorViewerAction::class)
        ->setName('mirador');
